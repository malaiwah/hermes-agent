...
class GatewayRunner:
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        try:
            ...
            if _is_voice_input:
                ...
                if not _primer_shown:
                    message_text = (
                        ...
                        "VOICE SELECTION: Emit a [voice: <id>] tag ONCE to switch "
                        "the active TTS voice for all subsequent auto-TTS in this "
                        "session. The tag is stripped before display.\n"
                        "  • Preset voices: ryan (m), aiden (m), dylan (m), eric (m), "
                        "vivian (f), serena (f), sohee (f, Korean), ono_anna (f, Japanese).\n"
                        "    Example: '[voice: vivian]\\nSure, switching to Vivian!'\n"
                        "  • Custom clones: use the vc_XXXX ID returned by "
            ...
            # Token counts are persisted by the agent directly into SessionDB
            # via _flush_messages_to_session_db. The gateway only tracks
            # last_prompt_tokens here for context-window tracking and
            # compression decisions. /status reads token totals from SessionDB
            # via SessionDB.get_session_token_totals — see
            # _handle_status_command. Adopted from upstream PR #5989 (which
            # also restored the upstream-correct update_session signature
            # after fork-local commit 1daa37bb had reintroduced the
            # accumulator pattern).
    ...
    async def _handle_voice_channel_join(
        self, event: MessageEvent, voice_prompt: str = None
    ) -> str:
        """Join the user's current Discord voice channel.

        ``voice_prompt`` is the text after ``/voice channel``, treated as the
        first message to the agent in the new voice session.  It can be
        anything natural — "greet everyone", "speak as Victor", "parle en
        français" — and the agent handles it with full tool access and session
        history.  When omitted a brief join-context marker is sent instead,
        ...
        if success:
            ...
            # ---- Greeting via the standard agent pipeline ----------------
            # Build a trigger message and dispatch it as a synthetic VOICE
            # MessageEvent — the same path live voice utterances take.  The
            # agent greets in voice, uses its tools (list_voice_clones, etc.),
            # and picks a voice clone if the prompt requests it.  The turn is
            # recorded in session history, giving the session a natural start.
            _vc_name = getattr(voice_channel, "name", "the voice channel")
            _summoner = getattr(event.source, "user_name", None) or ""
            _src_name = None
            _src_kind = "channel"
            try:
            ...
            try:
                for _m in getattr(voice_channel, "members", []):
                    _mn = getattr(_m, "display_name", None) or getattr(_m, "name", None)
                    if _mn:
            ...
            if voice_prompt:
                # User provided explicit intent — pass it straight through.
                _trigger_text = voice_prompt
            else:
                # No prompt — send a join-context marker so the agent knows
                # to generate a greeting without further instruction.
                _ctx_parts = [f"[Voice channel joined: {_vc_name}"]
                if _summoner:
                    _ctx_parts[0] += f", invited by {_summoner}"
                if _src_name:
                    _ctx_parts[0] += f" from the {_src_name} {_src_kind}"
                if _vc_members:
                    _ctx_parts.append(f"Members: {', '.join(_vc_members)}")
                _trigger_text = ". ".join(_ctx_parts) + ".]"

            from types import SimpleNamespace as _SNS
            _greeting_event = MessageEvent(
                source=event.source,
                text=_trigger_text,
                message_type=MessageType.VOICE,
                # raw_message carries guild_id so _get_guild_id() and the
                # voice-reply path can find the right voice channel.
                raw_message=_SNS(guild_id=guild_id, guild=None),
            )

            async def _do_greeting():
                try:
                    await adapter.handle_message(_greeting_event)
                except Exception as _e:
                    logger.warning("Voice greeting turn failed: %s", _e)

            _greeting_task = asyncio.create_task(_do_greeting())
    ...
    async def _run_agent(
        ...
        try:
            ...
            hidden_entry = None
            if session_key:
                hidden_entry = getattr(self, "_pending_hidden_turns", {}).pop(
                    session_key,
                    None,
                )
...
def _cleanup_orphaned_containers():
    ...
    if not docker_exe:
        _logger.debug("Docker/podman not found, skipping container cleanup")
        return
    ...
    try:
        ...
        if result.returncode != 0:
            _logger.debug("Could not list containers: %s", result.stderr)
            return
            ...
            if len(parts) >= 2:
                ...
                if status.lower() in ('exited', 'dead', 'created'):
                    try:
                        ...
                        exited_count += 1
                        _logger.info("Cleaned up orphaned container: %s (%s)",
                                     container_id[:12], status)
                    except Exception as e:
                        _logger.debug("Failed to remove container %s: %s",
                                      container_id[:12], e)

        if exited_count > 0:
            _logger.info("Startup cleanup: removed %d orphaned hermes-* containers",
                         exited_count)
        else:
            _logger.debug("No orphaned containers found")

    except Exception as e:
        _logger.warning("Container cleanup failed: %s", e)
